...
import com.alexbui.nutritrack.data.nutricoach.*
import com.alexbui.nutritrack.data.patient.Patient
import com.alexbui.nutritrack.R
import com.alexbui.nutritrack.ui.theme.DeepGreen
...
fun NutriCoachContent() {
    ...
    Column(
        ...
        if (isFruitScoreOptimal) {
            ...
            Button(
                onClick = {
                    if (fruitQuery.isNotBlank()) {
                        coachViewModel.fetchFruitInfo(fruitQuery.trim())
                    }
                },
                modifier = Modifier.align(Alignment.CenterHorizontally),
            ...
            if (coachViewModel.isLoading.value) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    CircularProgressIndicator(modifier = Modifier.size(20.dp))
                    Spacer(modifier = Modifier.width(12.dp))
                    Text("Fetching fruit info...")
                }
            }

            //show error from API (e.g. fruit not found)
            coachViewModel.fruitError.value?.let { errorMsg ->
                Card(
                    modifier = Modifier.fillMaxWidth(),
                    shape = RoundedCornerShape(12.dp),
